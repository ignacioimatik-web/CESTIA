...
const CONFIG: SupermarketConfig = {
  name: 'lidl',
  displayName: 'Lidl',
  logoUrl: '/images/supermarkets/lidl.svg',
  color: '#0050AA',
...
export class LidlProvider extends SupermarketProvider {
  ...
  async syncProducts(): Promise<SyncResult> {
    return { synced: 0, updated: 0, failed: 0, errors: ['Lidl provider: not yet implemented'], totalApiRequests: 0, categoriesProcessed: 0, productsFetched: 0, durationMs: 0 }
  }
...
let _instance: LidlProvider | null = null

export function registerLidl(): LidlProvider {
  if (!_instance) {
    _instance = new LidlProvider()
